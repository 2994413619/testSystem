...
function deletechoose(element){
	...
	$("#optionCount").attr("value",chooseSize);
}
...
function addChoose(element){
	...
	if(ChooseSize < 26){
		...
		//2、添加选项
		$("#QuestionStep").append("<div class='form-group' id='choose"+ChooseIndex+"'>"
												+"<div class='col-sm-8'>"
													+"<div class='input-group'>"
														+"<div class='input-group-addon'>"+ChooseIndex+"</div>"
														+"<input type='text' class='form-control' autocomplete='off' placeholder='题目选项' name='"+ChooseIndex+"'>"
													+"</div>"
												+"</div>"
												+"<img src='"+contextPath+"/img/delete_1.png' onmouseover='deleteChooseImgOver(this)' onmouseout='deleteChooseImgOut(this)' onclick='deletechoose(this)' forDivId='choose"+ChooseIndex+"' data-toggle='tooltip' data-placement='top' title='删除该选项' style='width: 34px;'/>"
											+"</div>");
		//3、添加正确答案
		$("#rightChoose div").append("<label class='checkbox-inline' id='rightchoose"+ChooseIndex+"'>"
									...
									+"</label>");
		$("#optionCount").attr("value",ChooseSize+1);
	} else {
		alert("最多只能有26个选项");
	}
